...
uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr initNodeWithDynamicID(const std::vector<std::string>& ifaces,
                                            ...
                                            const std::string& name)
{
    ...
    {
        const auto app_id = uavcan_posix::makeApplicationID<uavcan_posix::MachineIDReader>(name, instance_id);
...
int main(int argc, const char** argv)
{
    try
    {
        ...
        uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr node = initNodeWithDynamicID(std::vector<std::string>(argv + 2, argv + argc),
