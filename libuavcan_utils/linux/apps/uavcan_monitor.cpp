...
public:
    explicit Monitor(uavcan_posix::Node<uavcan_posix::SystemClock>& node)
...
static uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr initNodeInPassiveMode(const std::vector<std::string>& ifaces, const std::string& node_name)
{
    auto node = uavcan_posix::Node<uavcan_posix::SystemClock>::create(ifaces, node_name.c_str(),
...
static void runForever(uavcan_posix::Node<uavcan_posix::SystemClock>& node)
{
    Monitor mon(node);
    ENFORCE(0 == mon.start());
    while (true)
...
int main(int argc, const char** argv)
{
    try
    {
        ...
        uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr node = initNodeInPassiveMode(iface_names, "org.uavcan.linux_app.node_status_monitor");
        runForever(*node);
        return 0;
    }
