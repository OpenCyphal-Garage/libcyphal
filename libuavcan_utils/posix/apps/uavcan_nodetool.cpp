...
#include <algorithm>
#include <iterator>
#include <uavcan_posix/uavcan_posix.hpp>
#include <uavcan/protocol/node_status_monitor.hpp>
#include "uavcan_posix/debug.hpp"
...
uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr initNode(const std::vector<std::string>& ifaces, uavcan::NodeID nid, const std::string& name)
{
    auto node = uavcan_posix::Node<uavcan_posix::SystemClock>::create(ifaces, 
                                             name.c_str(),
...
typename DataType::Response call(uavcan_posix::BlockingServiceClient<DataType>& client,
...
const std::map<std::string,
               std::pair<std::string,
                         std::function<void(const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr&, const uavcan::NodeID,
...
{
    {
        "param",
        {
            ...
            [](const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr& node, const uavcan::NodeID node_id, const std::vector<std::string>& args)
            {
    ...
    },
    {
        "param_save",
        {
            ...
            [](const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr& node, const uavcan::NodeID node_id, const std::vector<std::string>&)
            {
    ...
    },
    {
        "hardpoint",
        {
            ...
            [](const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr& node, const uavcan::NodeID, const std::vector<std::string>& args)
            {
...
void runForever(const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr& node)
{
...
int main(int argc, const char** argv)
{
    try
    {
        ...
        uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr node = initNode(iface_names, self_node_id, "org.uavcan.linux_app.nodetool");
        runForever(node);
        return 0;
    }
