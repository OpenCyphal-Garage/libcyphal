...
using SocketCanIface = uavcan_posix::SocketCanIface;
using SystemClock = uavcan_posix::SystemClock;
using ClockAdjustmentMode = uavcan_posix::ClockAdjustmentMode;
using SocketCanDriver = uavcan_posix::SocketCanDriver;
using SocketCanError = uavcan_posix::SocketCanError;
...
{
    return SystemClock().getMonotonic() + uavcan::MonotonicDuration::fromMSec(ms);
}

static void testNonexistentIface()
{
    const int sock1 = SocketCanIface::openSocket("noif9");
    ENFORCE(sock1 < 0);
    const int sock2 = SocketCanIface::openSocket("verylongifacenameverylongifacenameverylongifacename");
    ENFORCE(sock2 < 0);
}

static void testSocketRxTx(const std::string& iface_name)
{
    const int sock1 = SocketCanIface::openSocket(iface_name);
    const int sock2 = SocketCanIface::openSocket(iface_name);
     ...
     */
    SystemClock clock_impl(ClockAdjustmentMode::PerDriverPrivate);
    clock_impl.adjustUtc(uavcan::UtcDuration::fromMSec(100000));
    const SystemClock& clock = clock_impl;

    SocketCanIface if1(clock, sock1);
    SocketCanIface if2(clock, sock2);
...
static void testDriver(const std::vector<std::string>& iface_names)
{
    ...
    for (int i = 0; i < driver.getNumIfaces(); i++)
    {
        for (auto kv : driver.getIface(i)->getErrors())
        {
            switch (kv.first)
            {
            case SocketCanError::SocketReadFailure:
            case SocketCanError::SocketWriteFailure:
            {
                ENFORCE(kv.second == 0);
                break;
            }
            case SocketCanError::TxTimeout:
            {
