 ...
 * a single, physical bus. Libcyphal shall use the first N interfaces it is capable of useing for redundancy.
...
class LIBCYPHAL_EXPORT InterfaceGroup
{
public:
     ...
     * @return libcyphal::Result::SuccessPartial if some but not all of the frames were written.
     *         libcyphal::Result::BufferFull if no frames could be written because the buffer was
     *         full.
     *         libcyphal::Result::Success if all frames were written.
     */
     ...
     * @return libcyphal::Result::Success If no errors occurred.
     */
     ...
     * libcyphal::Result::NotImplemented.
     *
     ...
     * @return libcyphal::Result::Success if the group's receive filtering was successfully reconfigured.
     *         libcyphal::Result::NotImplemented if this media layer implementation does not support
     ...
     *
     * @return  libcyphal::Result::SuccessTimeout if no events ocurred but the select operation timedout.
     *          libcyphal::Result::Success if one or more of the provided interfaces are ready for read, and if
     *          @p ignore_write_available is false, or write.
     *          libcyphal::Result::SuccessPartial if one or more errors were reported for one or more interfaces.
     */
    virtual libcyphal::Result select(libcyphal::duration::Monotonic timeout, bool ignore_write_available) = 0;
};
 ...
 * is not specified by libcyphal. Manager object shall remain valid for the as long as any other object in
 * libcyphal and all memory owned by the manager shall remain valid after startInterfaceGroup has been called
...
class LIBCYPHAL_EXPORT InterfaceManager
{
public:
    ...
    /**
     * Called by libcyphal when it is starting to use the group of interfaces managed by this object.
     * Libcyphal shall only invoke this once initially and only ever again if stopInterfaceGroup was first successfully
     * invoked.
     * @param  filter_config         The filtering to apply equally to all members of the group.
     * @param  filter_config_length  The length of the @p filter_config argument.
     * @param  out_group             A pointer to set to the started group. This will be nullptr if the start method
     *                               fails.
     * @return libcyphal::Result::Success if the group was successfully started and a valid pointer was returned.
     *         libcyphal::Result::SuccessParital can be returned to signal that a desired level of redundancy was not
     *         achieved but libcyphal may not modify its behavior based on this signal.
     *         The caller should assume that @p out_group is an invalid pointer if any failure is returned.
     */
    virtual libcyphal::Result startInterfaceGroup(const typename InterfaceGroupType::FrameType::Filter* filter_config,
                                                  std::size_t            filter_config_length,
                                                  InterfaceGroupPtrType& out_group) = 0;

    /**
     * Called by libcyphal when it is finished with the interface group. Libcyphal shall not invoke any further methods
     ...
     * method. It is provided solely to create a "hands-off" contract between libcyphal and a media layer
     * implementation.
     * @return libcyphal::Result::Success. Libcyphal will not respond to any failures reported by this method.
     */
    virtual libcyphal::Result stopInterfaceGroup(InterfaceGroupPtrType& inout_group) = 0;
