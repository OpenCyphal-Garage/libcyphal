...
static uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr initNode(const std::vector<std::string>& ifaces, uavcan::NodeID nid,
...
static void runForever(const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr& node)
{
...
int main(int argc, const char** argv)
{
    try
    {
        ...
        uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr node = initNode(iface_names, self_node_id, "org.uavcan.linux_test_node_status_monitor");
        runForever(node);
        return 0;
    }
