...
{
    const auto app_id = uavcan_posix::makeApplicationID<uavcan_posix::MachineIDReader>(name,  nid.get());

    uavcan::protocol::HardwareVersion hwver;
    std::copy(app_id.begin(), app_id.end(), hwver.unique_id.begin());
    std::cout << hwver << std::endl;

    auto node = uavcan_posix::Node<uavcan_posix::SystemClock>::create(ifaces, name.c_str(), uavcan::protocol::SoftwareVersion(), hwver, nid);
...
private:
    ...
    uavcan_posix::SystemClock clock_;
...
void redraw(const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr& node,
            ...
            const uavcan::dynamic_node_id_server::DistributedServer& server)
{
     ...
     * Collecting the data
     */
    const unsigned num_rows = uavcan_posix::getTerminalRows();
