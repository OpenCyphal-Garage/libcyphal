...
if(LCOV)
    ...
    set(LIBCYPHAL_GCOV_TOOL_ARG )

    if (${LIBCYPHAL_USE_LLVM_COV})
        ...
        if (LLVM_COV)
            ...
            file(COPY ${CMAKE_CURRENT_BINARY_DIR}/${CMAKE_FILES_DIRECTORY}/gcov_tool.sh
                DESTINATION ${LIBCYPHAL_NATIVE_TEST_BINARY_DIR}
                ...
                FILE_PERMISSIONS OWNER_READ
                                ...
                                WORLD_READ
                                WORLD_EXECUTE)
            set(LIBCYPHAL_GCOV_TOOL_ARG "--gcov-tool" "${LIBCYPHAL_NATIVE_TEST_BINARY_DIR}/gcov_tool.sh")
        else()
        ...
        add_custom_command(
            ...
            COMMAND # Generate initial "zero coverage" data.
                ${LCOV}
                        ...
                        --initial
                        --directory ${CMAKE_CURRENT_BINARY_DIR}
                        --output-file ${LIBCYPHAL_NATIVE_TEST_BINARY_DIR}/coverage.baseline.info
            COMMAND
            ...
            COMMAND # Combine all the test runs with the baseline
                ${LCOV}
                        ${LIBCYPHAL_GCOV_TOOL_ARG}
                        --rc lcov_branch_coverage=1
                        --add-tracefile ${LIBCYPHAL_NATIVE_TEST_BINARY_DIR}/coverage.baseline.info
                        --add-tracefile ${ARG_OUTDIR}/coverage.${ARG_TEST_NAME}.test.info
                        --output-file ${ARG_OUTDIR}/coverage.${ARG_TEST_NAME}.info
            COMMAND # Only use data for things under libcyphal.
                ${LCOV}
                        ${LIBCYPHAL_GCOV_TOOL_ARG}
                        --rc lcov_branch_coverage=1
