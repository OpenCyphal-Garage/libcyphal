...
TEST_F(TestClient, request_response_get_fetch_result)
{
    ...
    scheduler_.scheduleAt(1s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 100ms, Service::Request{{&mr_}}, now() + 2s);
...
TEST_F(TestClient, request_response_via_callabck)
{
    ...
    scheduler_.scheduleAt(1s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 100ms, Service::Request{{&mr_}}, now() + 500ms);
        EXPECT_THAT(maybe_promise, VariantWith<SvcResPromise>(_));
        response_promise.emplace(cetl::get<SvcResPromise>(std::move(maybe_promise)));
    });
    scheduler_.scheduleAt(1s + 300ms, [&](const auto&) {
...
TEST_F(TestClient, request_response_set_callabck_after_reception)
{
    ...
    scheduler_.scheduleAt(1s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 100ms, Service::Request{{&mr_}}, now() + 200ms);
        EXPECT_THAT(maybe_promise, VariantWith<SvcResPromise>(_));
        response_promise.emplace(cetl::get<SvcResPromise>(std::move(maybe_promise)));
    });
...
TEST_F(TestClient, request_response_failures)
{
    ...
    scheduler_.scheduleAt(1s, [&](const auto&) {
        //
        Service::Request request{{&mr_}};
        request.some_stuff = Service::Request::_traits_::TypeOf::some_stuff{{&mr_}};
    ...
    scheduler_.scheduleAt(2s, [&](const auto&) {
        ...
        const auto maybe_promise = client.request(now() + 100ms, Service::Request{{&mr_}});
    ...
    scheduler_.scheduleAt(3s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 100ms, Service::Request{{&mr_}});
        EXPECT_THAT(maybe_promise, VariantWith<SvcResPromise>(_));
        response_promise.emplace(cetl::get<SvcResPromise>(std::move(maybe_promise)));
    ...
    scheduler_.scheduleAt(5s, [&](const auto&) {
        ...
        const auto maybe_promise1 = client.request(now() + 100ms, Service::Request{{&mr_}});
        EXPECT_THAT(maybe_promise1, VariantWith<SvcResPromise>(_));

        const auto maybe_promise2 = client.request(now() + 100ms, Service::Request{{&mr_}});
        EXPECT_THAT(maybe_promise2, VariantWith<SvcResPromise>(_));

        const auto maybe_promise3 = client.request(now() + 100ms, Service::Request{{&mr_}});
...
TEST_F(TestClient, multiple_requests_responses_expired)
{
    ...
    scheduler_.scheduleAt(1s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 100ms, Service::Request{{&mr_}}, now() + 4s);
        EXPECT_THAT(maybe_promise, VariantWith<SvcResPromise>(_));
    ...
    scheduler_.scheduleAt(2s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 200ms, Service::Request{{&mr_}}, now() + 2s);
        EXPECT_THAT(maybe_promise, VariantWith<SvcResPromise>(_));
    ...
    scheduler_.scheduleAt(3s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 300ms, Service::Request{{&mr_}}, now() + 1s);
        EXPECT_THAT(maybe_promise, VariantWith<SvcResPromise>(_));
    ...
    scheduler_.scheduleAt(4s, [&](const auto&) {
        ...
        auto maybe_promise = client.request(now() + 400ms, Service::Request{{&mr_}}, now() + 2s);
        EXPECT_THAT(maybe_promise, VariantWith<SvcResPromise>(_));
