...
static uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr initMainNode(const std::vector<std::string>& ifaces, uavcan::NodeID nid,
                                          const std::string& name)
{
    std::cout << "Initializing main node" << std::endl;

    auto node = uavcan_posix::Node<uavcan_posix::SystemClock>::create(ifaces, name.c_str(),
...
static uavcan_posix::SubNode<uavcan_posix::SystemClock>::Ptr initSubNode(unsigned num_ifaces, uavcan::INode& main_node)
{
    std::cout << "Initializing sub node" << std::endl;

    typedef VirtualCanDriver<QueuePoolSize> Driver;

    std::unique_ptr<Driver> driver(new Driver(num_ifaces));

    main_node.getDispatcher().installRxFrameListener(driver.get());

    auto node = uavcan_posix::SubNode<uavcan_posix::SystemClock>::create(std::move(driver), main_node.getNodeID());

    return node;
}

static void runMainNode(const uavcan_posix::Node<uavcan_posix::SystemClock>::Ptr& node)
{
...
static void runSubNode(const uavcan_posix::SubNode<uavcan_posix::SystemClock>::Ptr& node)
{
