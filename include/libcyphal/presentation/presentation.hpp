...
{
public:
    /// @brief Defines a failure type for various `make...` methods of the presentation layer.
    ///
    ...
    ///
    /// The server must never outlive this presentation object.
    ///
    ...
    ///
    /// The client must never outlive this presentation object.
    ///
    ...
    /// @brief Sets an external transfer ID map provided by the user.
    ///
    /// By default, this presentation layer object operates without a transfer ID map.
    /// It means that continuous transfer IDs are not guaranteed across recreation of entities.
    /// In this mode, every new transmission session will start with the default (zero) initial transfer ID.
    /// Such default behavior is suitable for most applications, especially when an application
    /// is completely initialized at the start, and does not create presentation entities dynamically.
    ///
    /// However, in some cases, it might be required to have continuous transfer IDs across recreation of entities.
    /// For long-running applications (like a daemon), it might be enough to have a transfer ID map in memory only.
    /// For short-running applications (like a CLI tool), it might be necessary to add a persistent storage.
    ///
    ...
    {
        transfer_id_map_ = transfer_id_map;
    }
...
private:
    ...
    transport::ITransferIdMap*                 transfer_id_map_;
