/// @file
/// IPv4 and IPv6 types and functions for the Cyphal transport layer.
///
/// @copyright
...
#ifndef LIBCYPHAL_NETWORK_IP_ADDRESS_HPP_INCLUDED
#define LIBCYPHAL_NETWORK_IP_ADDRESS_HPP_INCLUDED
...
/// The Address class represents an IPv4 Address...for now. We're going to pretend this is either/or v4/v6 for now.
/// When we actually support v6 we'll extend this type to support both/either even if that means it's just a variant
/// with this type as the v4 type in it. Given that we're dealing with either a 32-bit or 128-bit value it's not
/// worth making this into an abstract object passed around by reference.
class Address final
{
public:
    ...
    constexpr Address(const Address& other) noexcept
        : Address(other.data_[3], other.data_[2], other.data_[1], other.data_[0])
    {
    }
    ...
    constexpr Address(Address&& other) noexcept
        : data_{other.data_[0], other.data_[1], other.data_[2], other.data_[3]}
    {
    }
    ...
    {
        return ((data_[3] & 0xF0) == 0b1110'0000);
    }
    ...
    {
        return static_cast<AddressType>((data_[3] << 24u) | (data_[2] << 16u) | (data_[1] << 8u) | (data_[0] << 0u));
    }
    ...
    constexpr const Octet* asOctets() const noexcept
    {
        return data_;
    }
...
#endif  // LIBCYPHAL_NETWORK_IP_ADDRESS_HPP_INCLUDED
